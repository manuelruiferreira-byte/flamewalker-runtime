...
export function frequencyUrgency({ usesThisWindow, daysLeftInWindow, targetUses7d, maxUses7d, minGapMet }, config = DEFAULT_FREQUENCY_CONFIG) {
  ...
  const slack = daysLeftInWindow - usesRemaining;
...
export function evaluateFrequencyPersistence(supplement, day, takenHistory = [], config = DEFAULT_FREQUENCY_CONFIG) {
  ...
  if (!supplement.available || supplement.autoSelection === 'excluded') {
    return Object.freeze({ engine: 'frequency_persistence', engineVersion: ENGINE_VERSION, supplementId: supplement.id,
      ...
      targetUses7d, maxUses7d, daysLeftInWindow: 0, lastTakenDate: null, reasonTrail: ['Excluded.'] });
  }
  if (supplement.autoSelection === 'manual_only') {
    return Object.freeze({ engine: 'frequency_persistence', engineVersion: ENGINE_VERSION, supplementId: supplement.id,
      state: FREQUENCY_STATES.MANUAL_ONLY, urgency: 0, minGapMet: false, usesThisWindow: 0,
      targetUses7d, maxUses7d, daysLeftInWindow: 0, lastTakenDate: null, reasonTrail: ['Manual-only.'] });
  }
  ...
  const base = { usesThisWindow, daysLeftInWindow, targetUses7d, maxUses7d, minGapMet };
  ...
  return Object.freeze({
    ...
    usesThisWindow, targetUses7d, maxUses7d, daysLeftInWindow, windowStart, windowEnd,
    lastTakenDate, minimumGapHours, residualWindowHours, persistenceClass: f.persistenceClass ?? 'unknown_conservative',
    reasonTrail: [reason]
  });
}
