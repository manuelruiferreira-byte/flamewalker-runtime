...
const diagnostics=Object.fromEntries(registry.supplements.map(item=>[item.id,{esotericLabel:'Compatible',esotericScalar:.5,convergenceBand:'Medium',convergenceCount:2,convergenceTotal:4,supportSystems:['astrology','bazi'],bodyPermission:'clear',frequencyState:'optional',frequencyUrgency:.2,usesThisWindow:1,targetUses7d:2,maxUses7d:3,minimumGapHours:24,residualWindowHours:24,eligibleOpportunitiesRemaining:3,pairingState:'complete'}]));
const record={date:'2026-06-17',historyMode:'actual',diagnostics,selected:[
...
assert.equal(model.authority,'INDIVIDUAL_OPTIMIZER_V3');
assert.equal(model.historyMode,'actual');
assert.equal(model.nadPrimary,'nr');
assert.equal(model.selected.filter(row=>['nr','nmn','nmnh'].includes(row.id)).length,1);
assert.equal(model.selected.find(row=>row.id==='nr').diagnostics.convergenceBand,'Medium');
...
console.log(JSON.stringify({ok:true,checks:12,nad:model.nadPrimary,authority:model.authority},null,2));
