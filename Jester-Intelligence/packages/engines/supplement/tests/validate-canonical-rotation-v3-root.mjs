...
const bazi=[['Wood','Yang'],['Wood','Yin'],['Fire','Yang'],['Fire','Yin'],['Earth','Yang'],['Earth','Yin'],['Metal','Yang'],['Metal','Yin'],['Water','Yang'],['Water','Yin']];
...
for(let i=0;i<28;i++){
  ...
  const [element,polarity]=bazi[i%bazi.length];
  const dayField={
    numerology:{scalar:.9,tags:[String(root)]},
    bazi:{scalar:.86,tags:[element,polarity]},
  ...
  for(const id of new Set(ids))(histories[id]??=[]).push(date);
}
...
for(let week=0;week<4;week++){
  const start=addDays('2026-06-15',week*7);
  const end=addDays(start,6);
  for(const [id,dates] of Object.entries(histories)){
    const card=byId.get(id);
    const count=dates.filter(date=>date>=start&&date<=end).length;
    assert(count<=Number(card?.frequency?.maxUses7d??7),`${id} respects maxUses7d in week ${week+1}`);
  }
}

for(const id of CANONICAL_EXCLUSION_GROUPS.weekly_limited_herbs){
  const dates=histories[id]??[];
  for(let i=1;i<dates.length;i++){
    const gap=(Date.parse(dates[i]+'T00:00:00Z')-Date.parse(dates[i-1]+'T00:00:00Z'))/86400000;
    assert(gap>=7,`${id} rolling uses are at least seven days apart`);
  }
}
